...
def move_ball(robot, from_pos, to_pos):
    robot.Movel_Cmd(POSE_LIFT_BALL_BACK_HIGH[from_pos], ARM_SPEED, 0, 0, 1)

    write_registers(robot, target_fingers[0], HAND_READY_GRASP_BALL_POS, ROH_ADDR, DELAY_TIME)

    robot.Movel_Cmd(POSE_LIFT_BALL_HIGH[from_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_GRASP_BALL[from_pos], ARM_SPEED, 0, 0, 1)

    write_registers(robot, target_fingers[0], HAND_GRASP_BALL_POS, ROH_ADDR, DELAY_TIME)

    robot.Movel_Cmd(POSE_LIFT_BALL_HIGH[from_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BALL_BACK_HIGH[from_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BALL_BACK_HIGH[to_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BALL_HIGH[to_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_GRASP_BALL[to_pos], ARM_SPEED, 0, 0, 1)
...
def move_bottle(robot, from_pos, to_pos):
    robot.Movel_Cmd(POSE_LIFT_BOTTLE_BACK_HIGH[from_pos], ARM_SPEED, 0, 0, 1)

    write_registers(robot, target_fingers[0], HAND_READY_GRASP_BOTTLE_POS, ROH_ADDR, DELAY_TIME)

    robot.Movel_Cmd(POSE_GRASP_BOTTLE[from_pos], ARM_SPEED, 0, 0, 1)

    write_registers(robot, target_fingers[0], HAND_GRASP_BOTTLE_POS, ROH_ADDR, DELAY_TIME)

    robot.Movel_Cmd(POSE_LIFT_BOTTLE_HIGH[from_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BOTTLE_BACK_HIGH[from_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BOTTLE_BACK_HIGH[to_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_LIFT_BOTTLE_HIGH[to_pos], ARM_SPEED, 0, 0, 1)
    robot.Movel_Cmd(POSE_GRASP_BOTTLE[to_pos], ARM_SPEED, 0, 0, 1)
...
def main():
    ...
    while True:
        ...
        robot.Movej_Cmd(POSE_INITIAL, 30, 0, 0, 1)
        robot.Movej_Cmd(POSE_DANCE, 30, 0, 0, 1)  # 手指舞角度
        ...
        robot.Movej_Cmd(POSE_VICTORY[POS_MIDDLE], 30, 0, 0, 1)  # 比耶

        write_registers(robot, target_fingers[0], HAND_VICTORY_POS, ROH_ADDR, DELAY_TIME)

        robot.Movej_Cmd(POSE_VICTORY[POS_LEFT], ARM_SPEED, 0, 0, 1)  # 比耶左
        robot.Movej_Cmd(POSE_VICTORY[POS_RIGHT], ARM_SPEED, 0, 0, 1)  # 比耶右
